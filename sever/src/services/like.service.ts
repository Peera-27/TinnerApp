...
export const LikeService = {
    togglelke: async function (user_id: string, target_id: string): Promise<boolean> {
    ...
    getFollower: async function (user_id: string, pagination: userPagination): Promise<userPaginator> {
        const _query = User.findById(user_id)
            .populate({
                ...
                select: '_id username display_name photos introduction location interest gender date_of_birth ',
